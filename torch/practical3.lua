...
local test_losses = {} -- added for test loss plot
local test_pred
...
for i = 1, iterations do
  ...
  test_pred = model:forward(test.data)
  test_losses[#test_losses + 1] = criterion(test_pred, test.labels) 
end
...
gnuplot.plot({
'training losses',torch.range(1, #losses),torch.Tensor(losses),'~'},
{'test_losses',torch.range(1, #test_losses),torch.Tensor(test_losses),'|'})
